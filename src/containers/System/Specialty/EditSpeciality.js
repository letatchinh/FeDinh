...
class EditSpeciality extends Component {
  ...
  handleDelteContentMarkdown = async () => {
   await axios.delete(`http://localhost:6969/api/delete-specialty?id=${this.state.selectedOption.item.id}`).then(res => {alert("Xoá Thành công") ;this.setState({selectedOption : null , contentMarkdown : "" , contentHTML : ""})})
   const res = await axios.get(`http://localhost:6969/api/get-specialty`);
   this.setState({
     listDoctors: res.data.data,
   });
